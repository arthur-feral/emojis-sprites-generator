 ...
 * @returns {Promise}
 */
const scrapEmojiPages = (categories) => {
  console.log('\n');
  let fullEmojis = [];
    ...
    return when.promise((resolve, reject) => {
      superagent.get(`${emojipediaMainUrl}${emoji.url}`)
        ...
        .end((error, result) => {
          ...
          emoji = _.merge(emoji, getEmojiDatasFromHtml(emoji, result.text, true));
          delete emoji.url;
      ...
      return when.promise((resolve, reject) => {
        superagent.get(`${emojipediaMainUrl}${extraEmoji.url}`)
          ...
          .end((error, result) => {
            ...
            extraEmoji = _.merge(extraEmoji, getEmojiDatasFromHtml(extraEmoji, result.text, false));
            delete extraEmoji.url;
