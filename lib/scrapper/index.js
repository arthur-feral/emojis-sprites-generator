...
const getEmojiDatasFromHtml = (emojiRaw, html, handleModifiers) => {
    ...
    if (config.fromCache) {
      ...
      superagent.get(emojipediaMainUrl)
        ...
        .end((error, result) => {
          ...
          try {
            saveHtmlPage(html, cachePath, 'index.html');
          } catch (error) {
            reject(error);
          }
...
const scrapEmojisPages = (emoji, index, resolve, html) => {
  emojisScrapped++;
  process.stdout.write(emojisScrapped + ' emojis scrapped \r');
  scrapEmojiPage(emoji, index, resolve, html);
};
const scrapExtraEmojisPages = (emoji, index, resolve, html) => {
  extraEmojisScrapped++;
  process.stdout.write(extraEmojisScrapped + ' extra emojis scrapped \r');
  scrapEmojiPage(emoji, index, resolve, html);
};
const scrapEmojiPage = (emoji, index, resolve, html) => {
  emoji = _.merge({
    themes: {},
    index: index
  }, emoji);

  emoji = _.merge(emoji, getEmojiDatasFromHtml(emoji, html, true));
  delete emoji.url;
  ...
  return when.all(_.map(emojis, (emoji, index) => {
    return when.promise((resolve, reject) => {
      superagent.get(`${emojipediaMainUrl}${emoji.url}`)
        ...
        .end((error, result) => {
          ...
          scrapEmojisPages(emoji, index, resolve, result.text);
        });
    });
  })).spread(function() {
    ...
    return when.all(_.map(extraEmojis, (extraEmoji, index) => {
      return when.promise((resolve, reject) => {
        superagent.get(`${emojipediaMainUrl}${extraEmoji.url}`)
          ...
          .end((error, result) => {
            ...
            scrapExtraEmojisPages(extraEmoji, index, resolve, result.text);
          });
      });
    })).spread(function() {
      ...
      _.each(fullEmojis, (fullEmoji) => {
        categories[fullEmoji.category].emojis.push(fullEmoji);
      });
  ...
  return when.promise((resolve, reject) => {
    gm(`${__dirname}/base.png`)
      ...
      .write(`${cachePath}/base.png`, (imageError) => {
          ...
          _.each(category.emojis, (emoji) => {
            _.each(emoji.themes, (url, themeName) => {
              ...
              imagesRequest.push(when.promise((resolve, reject) => {
                let themePath = `${`${cachePath}/images`}/${themeName}`;
                try {
        ...
        resolve(
          when.all(imagesRequest).spread(function() {
