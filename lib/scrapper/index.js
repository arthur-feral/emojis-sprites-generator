...
module.exports = (superagent, crawler, logger) => {
  ...
  const scrapEmojis = (forceCache, categories) => {
    ...
    return when.all(_.map(emojis, (emoji, index) => {
      ...
      extraEmojis = _.chain(fullEmojis)
        ...
        .flatten().value();
      logger.success(`${arguments.length} saved ${os.EOF}`);
      logger.info(`getting ${extraEmojis.length} emojis with modifiers  ${os.EOF}`);
  ...
  const scrap = (config) => {
    if (config.fromCache) {
      try {
        logger.info(`Loading ${config.destination}/emojis.json... ${os.EOF}`);
        const datas = fs.readFileSync(`${config.destination}/emojis.json`, 'utf8');
        return when.resolve(JSON.parse(datas));
      } catch (error) {
        logger.warn(`Cannot find emojis.json Crawling datas... ${os.EOF}`);
      }
    }
    return scrapIndex(config.fromCache)
      ...
      .then((datas) => {
        ...
        logger.success(`Successfully writen emojis json file. ${os.EOF}`);
  ...
  const scrapImages = (config, datas) => {
    ...
    return when.promise((resolve, reject) => {
      ...
      logger.info(`getting ${imagesRequests.length} images... ${os.EOF}`);
      resolve(
        when.all(imagesRequests).spread(function() {
          logger.success(`${imagesDone} images writen ${os.EOF}`);
