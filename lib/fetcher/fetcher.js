        ...
        superagent.get(BASE_URL)
          .end((error, result) => {
            if (error) {
              ...
              emitter.emit(FETCHER_FETCH_CATEGORIES_SUCCESS, result.text);
              resolve(result.text);
            }
          });
      });
        ...
        superagent.get(category.url)
          .end((error, result) => {
            if (error) {
              ...
              emitter.emit(FETCHER_FETCH_CATEGORY_SUCCESS, category, result.text);
              resolve(result.text);
            }
          });
      });
        ...
        superagent.get(emoji.url)
          ...
          .end((error, result) => {
            if (error) {
              ...
              emitter.emit(FETCHER_FETCH_EMOJI_SUCCESS, category, emoji, result.text);
              resolve(result.text);
            }
          });
      });
        ...
        superagent.get(url)
          ...
          .end((error, result) => {
            if (error || !result.body) {
              ...
              emitter.emit(FETCHER_FETCH_IMAGE_SUCCESS, category, emoji, themeName, result.body);
              resolve(result.body);
            }
          });
      });
