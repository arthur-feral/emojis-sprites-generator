...
describe('Fetcher', () => {
  ...
  describe('#fetchCategory', () => {
    it('fetch categories data from main page', async () => {
      ...
      const categories = await fetcher.fetchCategory({
  ...
  describe('#fetchEmoji', () => {
    it('fetch categories data from main page', async () => {
      ...
      const categories = await fetcher.fetchEmoji(
        {
          'fullName': 'Smileys & People',
          'name': 'people',
          'symbol': '😃',
      ...
      expect(categories.status).to.deep.equal(200);
      expect(categories.type).to.deep.equal('text/html');
      expect(categories.request.url).to.equal('https://emojipedia.org/reversed-hand-with-middle-finger-extended/');
