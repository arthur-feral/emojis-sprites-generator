...
module.exports = () => {
  ...
  const crawlCategoryPages = (category, html) => {
    categoriesCrawled++;
    process.stdout.write(categoriesCrawled + ' categories scrapped \r');
  ...
  const crawlEmojisPages = (emoji, index, html, isModifier) => {
    if (isModifier) {
      extraEmojisCrawled++;
      process.stdout.write(extraEmojisCrawled + ' extra emojis crawled \r');
    } else {
      emojisCrawled++;
      process.stdout.write(emojisCrawled + ' emojis crawled \r');
    }
