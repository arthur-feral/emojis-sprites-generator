...
export default (config, emitter) => {
   ...
   * @return {*}
   */
  const generateImage = (emoji, themeName, imageContent) => {
    imagesProcessing += 1;
    ...
    if (alreadyProcessed) {
      ...
      return new Promise((resolve, reject) => {
        jimp.read(imageRawPath).then((image) => {
          ...
          emitter.emit(PARSER_PARSE_IMAGE_ERROR, error, emoji, themeName, imageContent);
        });
      });
    }
  };
  ...
  const queueImageProcessing = (emoji, themeName, imageContent) => {
    if (imagesProcessing < MAX_IMAGES_TO_PROCESS_AT_TIME) {
      generateImage(emoji, themeName, imageContent);
    } else {
      queueToProcess.push([emoji, themeName, imageContent]);
    }
  };
  ...
  emitter.on(PARSER_PARSE_IMAGE_ERROR, (error, emoji, themeName, imageContent) => {
