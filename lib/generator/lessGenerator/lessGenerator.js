...
const generate = (theme, prefix, emojis, dimensions, destination) => {
  return when.promise((resolve, reject) => {
    ...
    _.each(emojis, (emo, index) => {
      lessContent += emoji(prefix, emo.shortname, -1 * (index * (dimensions.height - 1))) + os.EOL;
      if (_.has(emo, 'modifiers')) {
        _.each(emo.modifiers, (modifier, i) => {
          lessContent += emoji(prefix, modifier.shortname, -1 * ((index + i + 1) * (dimensions.height - 1))) + os.EOL;
        });
      }
    });
