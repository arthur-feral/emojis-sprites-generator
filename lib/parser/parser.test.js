...
import {
  FETCHER_FETCH_CATEGORIES_SUCCESS, FETCHER_FETCH_CATEGORY_SUCCESS,
...
const emojiParsed = require('./fixtures/emojiParsed.json');
...
// const emojiWithoutShortnamesHTML = fs.readFileSync(`${process.cwd()}/tests/html/emojiWithoutShortnames.html`, 'utf8');

// const emojiWithModifiers = fs.readFileSync(`${process.cwd()}/tests/html/emojiWithModifiers.html`, 'utf8');

describe('Parser', () => {
  describe('FETCHER_FETCH_CATEGORIES_SUCCESS', () => {
    it('parse categories data from main page', () => {
      expect(parseCategoriesSuccessSpy.callCount).to.equal(0);
      emitter.emit(FETCHER_FETCH_CATEGORIES_SUCCESS, indexHTML);
  ...
  describe('FETCHER_FETCH_CATEGORY_SUCCESS', () => {
    it('parse category data from category page', () => {
      expect(parseCategorySuccessSpy.callCount).to.equal(0);
      emitter.emit(FETCHER_FETCH_CATEGORY_SUCCESS, categoriesJSON.people, categoryHTML);

      expect(parseCategorySuccessSpy.callCount).to.equal(1);
      expect(parseCategorySuccessSpy.args[0][0]).to.deep.equal(_.map(categoryEmojis.people.emojis, emoji => emoji));
    });
  });

  describe('FETCHER_FETCH_EMOJI_SUCCESS', () => {
    beforeEach(() => {
    ...
    describe('simple emoji', () => {
      it('parses the emoji page', () => {
        ...
        emitter.emit(FETCHER_FETCH_EMOJI_SUCCESS, categoryEmojis.people.emojis[0], emojiHTML1);
    ...
    describe('emoji with long shortname', () => {
      it('parses the emoji page', () => {
        expect(parseEmojiSuccessSpy.callCount).to.equal(0);
        emitter.emit(FETCHER_FETCH_EMOJI_SUCCESS, {
          'symbol': '😙',
